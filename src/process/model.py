...
class Conv(nn.Module):
    ...
    def forward(self, hidden, x, batch):
...
class Readout(nn.Module):
    def __init__(self, emb_size, hidden_dim):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(hidden_dim + emb_size, 256), nn.ReLU(),
    ...
    def forward(self, h, x, batch):
        feature = torch.concat([h, x], dim = -1)
        feature = self.mlp(feature)
        global_feature = global_add_pool(feature, batch)
        global_feature = self.activation(self.output(global_feature).squeeze())
        return global_feature

class Net(nn.Module):
    def __init__(self, gated_graph_conv_args, conv_args, emb_size, max_nodes, device, gather_layer_type, classify_layer_type):
        super(Net, self).__init__()
        
        self.ggrn = GatedGraphConv(**gated_graph_conv_args).to(device)
        ...
        self.gather_layer = self.gather[gather_layer_type]
        self.classify_layer = self.classify[classify_layer_type]
        
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x = self.gather_layer(x, edge_index)
        x = self.classify_layer(x, data.x, data.batch)
